...
ROS is built and tested for Ubuntu Linux. It is not officially supported on macOS, and trying to install it directly on your Mac is a common source of
wasted afternoons. The standard fix — what real robotics labs do on Mac-based dev machines  is to run ROS **inside a Docker container**, and use VS Code's "Dev Containers" extension to edit and run code inside it as
if it were local. 

- This works identically whether your 2022 MacBook is
Apple Silicon (M1 Pro/M2) or Intel; Docker pulls the right image for your chip automatically.
...
Open a terminal in VS Code (`` Ctrl+` ``) and run ROS's own built-in demo nodes, just to confirm the environment itself works before we touch any of
our own code:
...
We skipped the `colcon build` / ROS package step entirely and just ran the `.py` files directly with `python3`. This works because `rclpy` is a normal
...
- That's the right trade for a first hello world; it's the first thing to unlearn once you move to a real project.
